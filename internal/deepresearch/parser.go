...
func ParseAnnualReport(pdfURL string) ([]SupplyChainEntity, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("pdf parser: marshal request: %w", err)
	}

	resp, err := parserHTTP.Post(parserURL()+"/parse", "application/json", bytes.NewReader(body))
	if err != nil {
		return nil, fmt.Errorf("pdf parser: request failed: %w", err)
	}
	defer resp.Body.Close()

	raw, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("pdf parser: read response: %w", err)
	}

	var result parseResponse
	if err := json.Unmarshal(raw, &result); err != nil {
		return nil, fmt.Errorf("pdf parser: decode response: %w", err)
	}
	if result.Error != "" {
