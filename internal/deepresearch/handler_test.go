...
func TestHandleDeepResearch_Success(t *testing.T) {
	...
	svc := NewService(nil, nil, &mockProvider{name: "NSE", reports: reports})
	r := newTestRouter(svc)
...
func TestHandleDeepResearch_AllProvidersFail(t *testing.T) {
	svc := NewService(nil, nil, &mockProvider{name: "NSE", err: errors.New("NSE unavailable")})
	r := newTestRouter(svc)
...
func TestHandleDeepResearch_SymbolNormalisedInResponse(t *testing.T) {
	svc := NewService(nil, nil, &mockProvider{name: "BSE", reports: []AnnualReport{}})
	r := newTestRouter(svc)
